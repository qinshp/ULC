package com.qinshp.agent;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

import com.qinshp.controller.ActionController;
...
public class Cell {
	...
	private void moveInFuTian() {
		ArrayList<Integer> inhabitantArray = this.devideFamilyNumber(this.getInhabitantNumber());// 获取每户的人数
		for (int i = 0; i < inhabitantArray.size(); i++) {
			...
			if (Parameter.availableHouse.size() > 0) {
				...
				Cell cell = ULC.searchCell(point.getX(), point.getY());
				if (cell != null) {
