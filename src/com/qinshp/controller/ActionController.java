...
import com.qinshp.agent.Family;
import com.qinshp.main.ULC;
...
public class ActionController {
	...
	public static void moveInFuTian(Family family, Point point, Context<Object> context, Grid<Object> grid) {
		for (int i = 0; i < ULC.cellTeam.size(); i++) {
			if (point.getX() == ULC.cellTeam.get(i).getX() && point.getY() == ULC.cellTeam.get(i).getY()) {
				for (int j = 0; j < family.getInhabitantTeam().size(); j++) {
					context.add(family.getInhabitantTeam().get(j));
					grid.moveTo(family.getInhabitantTeam().get(j), ULC.cellTeam.get(i).getX(),
							ULC.cellTeam.get(i).getY());
				}
				ULC.cellTeam.get(i).getFamilyTeam().add(family);
				ULC.cellTeam.get(i).setMoveInFamilyNumber(ULC.cellTeam.get(i).getMoveInFamilyNumber() + 1);
				ULC.cellTeam.get(i).setMoveInFamilyNumber_now(ULC.cellTeam.get(i).getMoveInFamilyNumber_now() + 1);
				context.add(family);
